...
|-----------|--------|--------|-------------|----------|
| ESLint (`--max-warnings 0`) | ✅ | ✅ | — | **자동** |
| TypeScript strict 타입 체크 | ✅ | ✅ | — | **자동** |
| Vite 프로덕션 빌드 | ✅ | ✅ | — | **자동** |
| Docker 이미지 빌드 | ✅ | ✅ | — | **자동** |
| SSH 헬스체크 (HTTP 200 확인) | — | — | ✅ | **자동** |
| Docker 컨테이너 상태 확인 | — | — | ✅ | **자동** |
| Nginx 액세스 로그 오류 확인 | — | — | ✅ | **자동** |
| `docker compose up --build` (로컬) | ⬜ | ⬜ | — | **수동** |
| UI 디자인/시각적 품질 판단 | ⬜ | — | ⬜ | **수동** |
| 브라우저 콘솔 오류 없음 확인 | ⬜ | ⬜ | ⬜ | **수동** |

### 자동 검증 전제 조건

- CI 자동 검증(ESLint/TypeScript/빌드/Docker)은 PR 생성 시 GitHub Actions에서 실행됩니다.
- deploy-prod 자동 검증은 main 배포 후 SSH로 실서버에서 실행합니다.
...
4. 실서버 자동 검증 (SSH 헬스체크, 컨테이너 상태, Nginx 로그)
...
ssh -i {SSH_KEY_PATH} {USER}@{SERVER_IP} \
  "cd {APP_PATH} && sudo docker compose ps"

# Nginx 액세스 로그 최근 오류 확인
ssh -i {SSH_KEY_PATH} {USER}@{SERVER_IP} \
  "cd {APP_PATH} && sudo docker compose logs nginx --tail 30 2>&1 | grep -i 'error\|warn' || echo 'No errors found'"
```
...
sudo docker pull ghcr.io/{GITHUB_ORG}/{PROJECT}:v{이전_버전}
...
```bash
ssh -i {SSH_KEY_PATH} {USER}@{SERVER_IP} \
  "cd {APP_PATH} && sudo docker compose down"

# 원인 조사 후 서비스 복구
ssh -i {SSH_KEY_PATH} {USER}@{SERVER_IP} \
  "cd {APP_PATH} && sudo docker compose up -d"
```
...
- [ ] XSS 방지 (React 기본 이스케이프 사용, `dangerouslySetInnerHTML` 사용 금지)
- [ ] 사용자 입력 값 검증 (URL, 날짜 등 외부 입력)
- [ ] `dangerouslyAllowBrowser` 사용 범위 최소화 (현재 claudeClient.ts 한정)

### 성능

- [ ] 불필요한 API 호출 없음 (useQuery 캐싱 활용)
- [ ] 무한 스크롤/페이지네이션 적용 (커밋 목록: 50건/페이지)
- [ ] useMemo/useCallback 과용 없음 (실제 비용이 있는 계산만 메모이제이션)

### 코드 품질

- [ ] TypeScript 타입 안전성 (`any` 사용 최소화, 타입 단언 지양)
- [ ] 에러 핸들링 (React ErrorBoundary, axios 에러 코드 분기)
- [ ] Draft/Applied 패턴 준수 (조회 버튼 클릭 시 apply 후 API 호출)
- [ ] Zustand 스토어에 민감 정보 평문 저장 시 경고

### 패턴 준수

- [ ] 프로젝트 컨벤션에 맞는 파일/디렉토리 구조
- [ ] API 클라이언트 추상화 레이어 사용 (`gitlabClient`, `redmineClient`, `claudeClient`)
- [ ] Vite 프록시 경로 사용 (`/gitlab-api`, `/redmine-api`) — 브라우저 직접 CORS 호출 금지

### CI 통과 확인

- [ ] `npm run lint` — ESLint 경고 0개
- [ ] `npm run typecheck` — TypeScript 오류 0개
- [ ] `npm run build` — 빌드 성공
...
| 기능/아키텍처 변경 | 섹션 8.5 트리거 참조 | sprint-close agent |
